...
class PaperTree:
    ...
    def __init__(
        ...
        section_max_tokens: int = settings.PAPER_TREE_CONFIG["section_max_tokens"], 
        keep_splitting: bool = True
    ):
        self.title = self._normalize_text(title)
        self.text = self._normalize_text(text)
        self.abstract = self._normalize_text(abstract)
    ...
    def _normalize_text(text: str) -> str:
        """
        Normalize text formatting to ensure consistent output.
        This removes excessive whitespace, normalizes line breaks, and cleans up formatting.
        """
        if not text:
            return text
    ...
    def get_root(self):
        """Get the root node for this paper tree"""
    ...
    def _extract_elements(self, text: str) -> tuple[str, List['PaperTree']]:
        ...
        for element_type in settings.LATEX_ELEMENT_TYPES:
    ...
    def _split_text_into_chunks(self, text: str) -> List['PaperTree']:
        ...
        if len(re.sub(r"\s+", "", text)) < settings.PAPER_TREE_CONFIG["min_section_length"]:
            return []
        ...
        if len(chunks) > settings.PAPER_TREE_CONFIG["max_chunks"]:
    ...
    def _create_subsections(self, text: str, chunks: List[str]) -> List['PaperTree']:
        ...
        num_sections = min(math.ceil(len(chunks) / (settings.PAPER_TREE_CONFIG["max_chunks"] - 1)), settings.PAPER_TREE_CONFIG["max_chunks"])
